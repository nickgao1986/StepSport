...
package com.pic.optimize.helper.view;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private int tabBackgroundResId = R.drawable.mt_background_tab;
    ...
    protected void addTextTab(final int position, String title) {
        addTextTab(position, title, false);
    }

    protected void addTextTab(final int position, String title, boolean hasRedPoint) {
        ...
        addTab(position, tab, hasRedPoint);
        textViews.add(position, tab);
    }



    private void addTab(final int position, View tab, boolean hasRedPoint) {
        ...
        if (widthNum != -1) {
            float mIndicatorWidth = (float) ScreenUtil.getScreenWidth(getContext()) / widthNum;
        ...
        for (int i = 0; i < textViews.size(); i++) {
            ...
            if (textAllCaps) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
                    ...
                    textViews.get(i).setText(textViews.get(i).getText().toString().toUpperCase(locale));
                }
            }
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isRoundCorner) {
            mRectF.left = lineLeft + (shouldExpand ? offset : tabPadding);
            mRectF.top = height - indicatorHeight - indicatorpadding;
            mRectF.right = lineRight - (shouldExpand ? offset : tabPadding);
            mRectF.bottom = height - indicatorpadding;
            canvas.drawRoundRect(mRectF, indicatorHeight, indicatorHeight, rectPaint);
        } else {
    ...
    protected void selectColerChange() {
